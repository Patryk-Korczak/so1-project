...
bool create_result_file(int cnt, int wait_time, int repeats); // create 'result.txt' file that will be appended by child process
...
int main(int argc, char** argv) {
	...
	if(argc == 1){
		...
		if(!output){
			cout << "Couldn't create output file, program will exit." << endl;
			return 1;
		}
		...
		for(int i = 0; i < repeats; i++){
			...
			if(!is_saved){
				return 1;
				cout << "Couldn't save file, program will now exit" << endl;
			}
			
			random_int_array.clear(); // clear vector after it's no longer needed
			...
			if(!process_start){
				cout<< "Couldn't execute child process, program will now exit." << endl;
				return 1;
			}
...
void print_array(vector <int> *random_int_array){
	for(unsigned i=0; i<random_int_array->size(); i++){
...
bool create_result_file(int cnt, int wait_time, int repeats){
    ...
    //create string that will be saved
